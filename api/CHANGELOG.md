...
## [0.7.4](https://github.com/CloudinaryLtd/public-interfaces/compare/api-v0.7.3...api-v0.7.4) (2026-07-22)
...
* **MCP:** Disable `createInitialBackup`, `listInitialBackups`, `getInitialBackup`, and `concat` from MCP tool generation (`disabled: true`).

## [0.7.2]() (2026-07-05)

_No public schema changes._

## [0.7.1]() (2026-06-16)

### Added

* **Metadata Fields:** `group` property (`string | null`) for organizing fields into collapsible UI sections.

## [0.7.0]() (2026-06-14)

### Fixed

* **Search:** `format`, `width`, `height`, `aspect_ratio`, and `pixels` declared nullable (`[type, 'null']`) in `search_response` to match actual API output for raw and audio assets.
* **Search:** `access_mode` declared nullable in `search_response`; added to the compat overlay for list endpoints.

## [0.6.0]() (2026-06-09)

### Added

* **Upload:** New `POST /v1_1/{cloud}/asset/download` endpoint (`downloadAssetByAssetId`) — downloads an asset identified by stable `asset_id` instead of the `resource_type/type/public_id` triplet.

### Changed

* **Upload:** Extended `X-Upload-Total-Parts` description to document that it may be omitted on non-terminal chunks once the session total is established; tightened `X-Upload-Part-Number` and `Content-Range` descriptions to match the explicit-order semantics.
